...
if [ ! -f beagle4.jar ] || [ ! -f beagle2vcf.jar ] || [ ! -f vcf2beagle.jar ]  || [ ! -f vcf2gprobs.jar ] || [ ! -f gprobsmetrics.jar ] || [ ! -f conform-gt.jar ]; then
...
#**** REF
 ...
 java -jar beagle4.jar gt=${outref}chr$i.vcf nsamples=10 burnin-its=10 phase-its=30 impute-its=30 out=${outref}chr$i.phased
...
#**** Impute
 java -jar beagle4.jar ref=${outref}chr$i.phased.vcf.gz gt=${outval}_chr$i.vcfmod.vcf.gz nsamples=10 burnin-its=10 phase-its=30 impute-its=30 out=${finaloutfile}chr$i.imputed
...
#**** REF
 java -jar beagle2vcf.jar $i ${outref}chr$i.snplist ${outref}_chr$i.bgl 0 > ${outref}chr$i.vcf
 java -jar beagle4.jar gt=${outref}chr$i.vcf window=$nloci nsamples=10 burnin-its=10 phase-its=30 impute-its=30 out=${outref}chr$i.phased
if [ ! -f ${outref}chr$i.phased.vcf.gz ]; then
 echo " Imputation with BEAGLE v4 aborted  "
 echo " Analysis now at on chromosome "$i
 echo " errrors were detectd, check the log file "
...
#**** VAL
 ...
 java -jar conform-gt.jar ref=${outref}chr$i.phased.vcf.gz gt=${outval}_chr$i.vcf chrom=$i out=${outval}_chr$i.vcfmod
#**** Impute
 java -jar beagle4.jar ref=${outref}chr$i.phased.vcf.gz gt=${outval}_chr$i.vcfmod.vcf.gz nsamples=10 burnin-its=10 phase-its=30 impute-its=30 window=$nloci out=${finaloutfile}chr$i.imputed
if [ ! -f ${finaloutfile}chr$i.imputed.vcf.gz ]; then
 echo " Imputation with BEAGLE v4 aborted  "
 echo " Analysis now at on chromosome "$i
 echo " errrors were detectd, check the log file "
